...
一个纯服务端的 PvP 匹配 Mod：支持 **1v1 / 2v2 / 自由乱斗(FFA) / 相扑(Sumo) / 空岛战争(SkyWars) / 战桥(Bridge) / 幸运之柱(Lucky Pillar) / TNT 跑酷(TNT Run) / 心跳水立方(Heartbeat) / 烫手山芋(Hot Potato) / 起床战争(Bed Wars) / 村庄保卫战(Village Defense) / 色盲派对(Colorblind Party) / 死斗(Deathmatch)** 共 18 种模式，支持**队列自动匹配**与**决斗挑战**，可自选**装备套件**（剑战 / 弓箭 / 全装备 / NoDebuff / 自定义 JSON 配置等），其中「1.8 经典」套件自带 **1.8.7 低版本战斗手感**。每场比赛在**专用虚空竞技场维度**中生成独立地形，结束后销毁重建。
...
### PvP 对战（自选套件）
- **1v1 / 2v2 / FFA / 相扑**，选择套件后加入队列
- **自由乱斗**：凑齐 3 人开始 60 秒倒计时，期间达到 6 人自动加速到 10 秒内开赛（人数不限，每人用自己的套件）
- **相扑**：不吃伤害只吃击退，落到平台下方 20 格淘汰（套件带一组末影珍珠可救回），最后一个留在场上的人获胜
- **「1.8 经典」套件**：模拟 1.8.7 手感——**无攻击冷却**（疯狂点按打满伤害）、剑**格挡减伤 50%**（右键格挡、攻击解除）、**没有剑横扫**、投掷物不继承投掷者速度。这套手感跟着**套件**走而不是模式走（见 `MatchManager.usesLegacyCombat`）：在自由乱斗 / 死斗 / 1v1 里选它，照样按 1.8.7 打，不会退回高版本 PvP。套件内容：钻石剑 + 全套铁甲 + 8 金苹果 + 速度 II
...
主菜单：
- 🗡 **PvP 对战** → 子菜单：**1v1 / 2v2 / FFA / 相扑 / 死斗** → 选择套件即加入队列
...
| `/pvp join <1v1\|2v2\|ffa\|sumo\|deathmatch> <套件>` | 加入匹配队列 |
...
内置套件 ID：`sword`（剑战）、`bow`（弓箭）、`full_gear`（全装备）、`iron_pvp`（铁套PVP）、`sumo`（相扑击退棍）、`no_debuff`（NoDebuff 药水PvP）、`gapple`（金苹果）、`axe`（斧战）、`legacy_1_8`（1.8 经典：钻石剑+全套铁甲+8金苹果+速度II，**自带 1.8.7 战斗手感**），以及 `kits.json` 中自定义的套件（用其 `name` 字段调用）。**空岛战争/战桥/趣味小游戏/起床战争/村庄保卫战/色盲派对不需要套件**；需要选套件的是 1v1 / 2v2 / 自由乱斗 / 相扑 / 死斗。
...
- **死亡幽灵**：**死亡不弹死亡界面**，直接转为**旁观者模式幽灵**——空物品栏（身上物品**爆落在地**供其他玩家拾取）、无敌、**隐身**、**可自由飞行**，**无法与对局任何交互**；靠近掉落物时会被**向上弹开**，等比赛结束自动恢复回主城。
