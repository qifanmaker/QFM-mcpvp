...
- **空岛战争 (Beta)**：每人随机一座出生空岛（3 箱），到**巨大的中间主岛**（半径约 30，5 箱）之间有**一座中途岛**（半径约 8、3 箱，作为搭桥跳板）；地图大幅放大、出生岛离主岛更远；**无套件、生存模式开箱搜装备**，采用 **1.8 低版本战斗**（无攻击冷却、剑可格挡减伤 50%）；武器/护甲随机附魔（低等级概率高）；**极低概率**开箱出 **鞘翅+3 烟花火箭、妙人斧（锋利 666 金斧·耐久 1）、附魔金苹果**；凑齐 4 人开赛（最少 2 人、最多 8 人），**3 分钟后缩圈**逼团（安全半径不小于主岛），最后幸存者获胜
...
- **空岛战争开箱**：出生岛 3 箱（铁装为主 + **保底一组搭桥方块** + 金苹果/末影珍珠，**低概率少量 TNT**），中间主岛箱子更富（钻石装为主，可能出 TNT/岩浆桶，TNT 已削弱）；武器/护甲随机附魔（**锋利/保护/力量 I 55% / II 30% / III 15%**）；中间岛每箱约 3% 概率出**极稀有物品**（鞘翅+3 烟花火箭、妙人斧、附魔金苹果各约 1%）。
