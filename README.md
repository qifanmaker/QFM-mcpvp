...
一个纯服务端的 PvP 匹配 Mod：支持 **1v1 / 2v2 / 自由乱斗(FFA) / 相扑(Sumo) / 1.8 经典PvP** 对战，支持**队列自动匹配**与**决斗挑战**，可自选**装备套件**（剑战 / 弓箭 / 全装备 / NoDebuff / 自定义 JSON 配置等）。每场比赛在**专用虚空竞技场维度**中生成独立平台，结束后销毁重建。

- **自由乱斗**：凑齐 3 人开始 60 秒倒计时，期间达到 6 人自动加速到 10 秒内开赛（人数不限，每人用自己的套件）
- **相扑**：不吃伤害只吃击退，被击出平台即淘汰，最后留在台上的人获胜
...
内置套件 ID：`sword`（剑战）、`bow`（弓箭）、`full_gear`（全装备）、`iron_pvp`（铁套PVP）、`sumo`（相扑击退棍）、`no_debuff`（NoDebuff 药水PvP）、`gapple`（金苹果）、`axe`（斧战）、`legacy_1_8`（1.8 经典：钻石剑+速度II+金苹果），以及 `kits.json` 中自定义的套件（用其 `name` 字段调用）。
...
- **右侧信息栏**：对局中屏幕右侧计分板实时显示模式、套件、各队存活人数。
