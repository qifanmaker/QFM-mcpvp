...
- **1.8 经典PvP**：模拟 1.8.9 手感——**无攻击冷却**（疯狂点按打满伤害）、剑**格挡减伤 50%**（右键格挡、攻击解除）、投掷物不继承投掷者速度。**「1.8 经典」套件自带这套手感**：在自由乱斗 / 死斗 / 1v1 里选它，同样按 1.8.7 打，不会退回高版本 PvP（见 `MatchManager.usesLegacyCombat`）。套件内容：钻石剑 + 全套铁甲 + 8 金苹果 + 速度 II
...
内置套件 ID：`sword`（剑战）、`bow`（弓箭）、`full_gear`（全装备）、`iron_pvp`（铁套PVP）、`sumo`（相扑击退棍）、`no_debuff`（NoDebuff 药水PvP）、`gapple`（金苹果）、`axe`（斧战）、`legacy_1_8`（1.8 经典：钻石剑+全套铁甲+8金苹果+速度II，**自带 1.8.7 战斗手感**），以及 `kits.json` 中自定义的套件（用其 `name` 字段调用）。**空岛战争/战桥/趣味小游戏/起床战争/村庄保卫战/色盲派对不需要套件**。
