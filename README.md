...
一个纯服务端的 PvP 匹配 Mod：支持 **1v1 / 2v2 / 自由乱斗(FFA) / 相扑(Sumo) / 1.8 经典PvP / 空岛战争(SkyWars)** 对战，支持**队列自动匹配**与**决斗挑战**，可自选**装备套件**（剑战 / 弓箭 / 全装备 / NoDebuff / 自定义 JSON 配置等）。每场比赛在**专用虚空竞技场维度**中生成独立地形，结束后销毁重建。
...
- **空岛战争 (Beta)**：每人随机一座出生空岛（3 箱）+ 地图中央主岛（箱子更富），**无套件、生存模式开箱搜装备**，采用 **1.8 低版本战斗**（无攻击冷却、剑可格挡减伤 50%）；武器/护甲随机附魔（低等级概率高）；**极低概率**开箱出 **鞘翅+3 烟花火箭、妙人斧（锋利 666 金斧·耐久 1）、附魔金苹果**；凑齐 4 人开赛（最少 2 人、最多 8 人），**3 分钟后缩圈**逼团，最后幸存者获胜
...
| `/pvp leave` | 离开队列 |
| `/pvp start` | OP 专用：立即用当前队列人数开赛（跳过倒计时） |
| `/pvp queue` | 查看自己排队状态 |
...
- **旁观者观战**：死亡后立即切换旁观者，快捷栏左侧**指南针**（右键切换观战目标）、中间**绿宝石**（立即重新匹配当前模式）、右侧**红石粉**（退出回主城）。
- **右侧信息栏**：对局中屏幕右侧计分板实时显示模式、套件、各队存活人数（空岛显示"开箱获得"）。
- **登录清空**：加入服务器时先清空背包再发放 UI 工具，不会出现背包挤满导致拿不到指南针。
- **空岛战争开箱**：出生岛 3 箱（铁装为主 + **保底一组搭桥方块** + 金苹果/末影珍珠，**中等概率出 TNT**），中间主岛箱子更富（钻石装为主，可能出 TNT/岩浆桶）；武器/护甲随机附魔（**锋利/保护/力量 I 55% / II 30% / III 15%**）；中间岛每箱约 3% 概率出**极稀有物品**（鞘翅+3 烟花火箭、妙人斧、附魔金苹果各约 1%）。
