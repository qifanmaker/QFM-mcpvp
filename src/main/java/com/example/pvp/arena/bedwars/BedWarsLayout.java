...
public final class BedWarsLayout {
    ...
    public static final class Team {
        ...
        public Formatting color;    // 队伍颜色（从岛屿方块探测，默认按索引）
        ...
        Team(int index, BlockPos bed, BlockPos spawn, BlockPos iron, BlockPos gold,
            ...
            this.color = TEAM_COLORS[Math.min(index, TEAM_COLORS.length - 1)];
        }
    }
    ...
    /** 解析颜色名（red/blue/...）为 Formatting，失败用默认。 */
    private static Formatting parseColor(String name, int fallbackIndex) {
        if (name == null) {
            return TEAM_COLORS[Math.min(fallbackIndex, TEAM_COLORS.length - 1)];
        }
        return switch (name.toLowerCase()) {
            ...
            case "pink", "light_purple" -> Formatting.LIGHT_PURPLE;
    ...
    /** 颜色 → 队伍名。 */
    public static String colorName(Formatting color) {
        return switch (color) {
            case RED -> "红队";
            case BLUE -> "蓝队";
            case YELLOW -> "黄队";
